...
const extractTypesMapUsingRegex = async (resultPath: string): Promise<Map<string, string[]>> => {
  ...
  const typeRegex = new RegExp(
    'type\\s+(?<typeName>\\w+)(?:\\s*:\\s*(?<parentType>\\w+))?\\s*(?:\\{(?<typeBody>[^}]*)\\}|;)\\s*',
    'gm'
  );
...
const executeConvertCommand = async (
  ticketNumber: string,
  cmlApiName: string,
  sfCommandStubs: ReturnType<typeof stubSfCommandUx>
) => {
...
describe('cml convert prod-cfg-rules', () => {
  ...
  it('tests W-19783372', async () => {
    ...
    expect(laptopProBundleTypeLines).to.include(
      'constraint lpb_gk_criteria_1 = ((laptop1[Laptop] > 0) && laptop1[Laptop].Memory == "RAM 64GB");'
    );
  ...
  it('tests W-19785067', async () => {
    ...
    const [laptopProductivityBundleType, laptopProductivityBundleTypeLines] = expectTypeAndGetLines(
      typesMap,
      'LaptopProductivityBundle'
    );
    ...
    expect(laptopProductivityBundleType).to.not.include(
      '"Disable", "attribute", "Windows_Processor", "value", ["i7-CPU 4.7GHz", "Intel Core i9 5.2 GHz"]);'
    );
    ...
    expect(laptopTypeLines).to.include(
      'rule(parent_lpb_vr_criteria_1_value == true, "Hide", "attribute", "Graphics");'
    );
    expect(laptopTypeLines).to.include(
      'rule(parent_lpb_vr_criteria_1_value == true, "Disable", "attribute", "Windows_Processor", "value", ["i7-CPU 4.7GHz", "Intel Core i9 5.2 GHz"]);'
    );
    ...
    expect(printerTypeLines).to.include(
      'rule(parent_lpb_vr_criteria_1_value == true, "Hide", "attribute", "Printer", "value", "Laser");'
    );
  });
  ...
  it('tests W-19786482', async () => {
    ...
    expect(desktopTypeLines).to.include(
  ...
  it('tests W-19996586', async () => {
    ...
    const smtAnnotationIndex = laptopProBundleTypeLines.findIndex((line) =>
      line.includes('@(tagName = "SellingModelType")')
    );
